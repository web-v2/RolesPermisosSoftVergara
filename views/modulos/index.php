...
<script src="dist/js/demo.js"></script>-->
      ...
      <script>
        ...
        $(document).ready(function() {
          $.ajax({
            url: "config/ajaxDatosModulos.php",
                  ...
                  "columns": [{
                    ...
                    },
                    {
                      ...
                      render: function(data, type, row) {
                        return "<a href='editar-modulo/" + data['idModulo'] + "' class='btn btn-sm btn-primary'><i class='fas fa-edit'></i> Editar</a>"
                      }
                    },
                    {
                      "data": null,
                      render: function(data, type, row) {
                        return "<a href='borrar-modulo/" + data['idModulo'] + "' class='btn btn-sm btn-danger'><i class='far fa-trash-alt'></i> Borrar</a>"
                      }
                    }
                  ],
                  bJQueryUI: true,
        ...
        $(document).ready(function() {
          $("#btnEnvio").on('click', function() {
            const nom = $('#nombres').val();

            if (nom == '' || nom.length == 0) {
              Command: toastr["warning"]("No puede ser nulo o vacío!", "Error, Campo Obligatorio")
              $('#nombres').addClass('bg-warning');
            }
            else {
              $('#nombres').removeClass('bg-warning');
              $('#myForm').submit();
            }
          });
        });
          ...
          <div class="modal-body">
            <div class="card">
              <div class="card-body">
                <p>
                  <center>
                    <form action="Modulos" method="post" id="myForm">
                      <input type="hidden" name="form" value="form-data">
                      <div class="col-md-12">
                        <label for="nombres" class="form-label">Nombre del Módulo</label>
                        <input type="text" class="form-control" name="nombres" id="nombres" required placeholder="Nombre del módulo a crear">
                      </div>
                      <div class="col-md-12">
                        <label for="detalles" class="form-label">Descripción del modulo</label>
                        <textarea name="descripcion" id="detalles" cols="10" rows="3" class="form-control" placeholder="Breve información acerca del módulo"></textarea>
                      </div>
                </p>
                </center>
...
<?php
    ...
    ";
        ...
        echo "
    <script>
        Command: toastr['error']('Detalles: $resp Comuniquese con el administrador del sistema!', 'Error, Fallo la inserción!');
    </script>
    ";
      }
    }
